...
def _connect_mongo(candidate_uris: Iterable[str], timeout_ms: int = 3000) -> tuple[MongoClient, str]:
...
def build_tam_tables(db, job_id: str) -> tuple[list[dict], list[dict], list[dict]]:
...
def _job_exists_in_ssdmt(db, job_id: str) -> bool:
    return db.segmentos_mt_tabular.count_documents({"job_id": job_id}, limit=1) > 0
...
def resolve_job_id(db, requested_job_id: str | None) -> str:
    ...
    if latest_job and latest_job.get("job_id") and _job_exists_in_ssdmt(db, latest_job["job_id"]):
...
def main() -> None:
    ...
    parser.add_argument("--job-id", default=os.getenv("JOB_ID"), help="Job ID a consultar")
    parser.add_argument(
        "--mongo-uri",
        default=os.getenv("MONGO_URI"),
        help="URI do MongoDB (se omitido, tenta hosts padrao)",
    )
    parser.add_argument("--mongo-db", default=os.getenv("MONGO_DB", "fatec_api"), help="Database")
    parser.add_argument(
        "--output-csv",
