...
def test_endpoint_criticidade_sucesso():
    ...
    with patch('backend.routes.criticidade.calcular_score_criticidade', return_value=mock_result):
        response = client.get('/etl/criticidade?ano=2024&distribuidora=EQUATORIAL')
...
def test_endpoint_criticidade_distribuidora_curta():
    ...
    assert response.status_code == 400
    assert 'Nome da distribuidora deve ter pelo menos 2 caracteres' in response.json()['detail']


def test_endpoint_criticidade_dados_nao_encontrados():
    """Testa endpoint quando não há dados para os parâmetros."""
    with patch('backend.routes.criticidade.calcular_score_criticidade', return_value=None):
        response = client.get('/etl/criticidade?ano=2024&distribuidora=INEXISTENTE')
        
        assert response.status_code == 404
        assert "Dados não encontrados para distribuidora 'INEXISTENTE' no ano 2024" in response.json()['detail']


def test_endpoint_criticidade_erro_interno():
    """Testa endpoint quando ocorre erro interno."""
    with patch('backend.routes.criticidade.calcular_score_criticidade', side_effect=Exception('Erro simulado')):
        response = client.get('/etl/criticidade?ano=2024&distribuidora=EQUATORIAL')
        
        assert response.status_code == 500
        assert 'Erro interno ao processar solicitação' in response.json()['detail']
...
def test_calcular_score_criticidade_sem_dados():
    ...
    with patch('backend.services.criticidade.buscar_dados_realizados', return_value=[]), \
         patch('backend.services.criticidade.buscar_dados_limites', return_value=[]):
...
def test_calcular_score_criticidade_com_dados():
    ...
    dados_realizados = [
        {'sig_agente': 'EQUATORIAL', 'ide_conj': '123', 'dsc_conj': 'TESTE', 'sig_indicador': 'DEC', 'valor_realizado': 110.0},
        {'sig_agente': 'EQUATORIAL', 'ide_conj': '123', 'dsc_conj': 'TESTE', 'sig_indicador': 'FEC', 'valor_realizado': 105.0}
    ]
    
    # Mock dados limites
    dados_limites = [
        {'sig_agente': 'EQUATORIAL', 'ide_conj': '123', 'dsc_conj': 'TESTE', 'sig_indicador': 'DEC', 'valor_limite': 100.0},
        {'sig_agente': 'EQUATORIAL', 'ide_conj': '123', 'dsc_conj': 'TESTE', 'sig_indicador': 'FEC', 'valor_limite': 100.0}
    ]
    
    with patch('backend.services.criticidade.buscar_dados_realizados', return_value=dados_realizados), \
         patch('backend.services.criticidade.buscar_dados_limites', return_value=dados_limites), \
         patch('backend.services.criticidade.salvar_score_criticidade') as mock_salvar:
